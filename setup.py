...
# Hack to get __version__ from snpkit/__init__.py
with open("snpkit/__init__.py") as f:
    for line in f:
...
include_dirs = [
    os.path.join("snpkit", "src"),
    os.path.join("snpkit", "src", "include"),
    os.path.join("snpkit", "src", "third_party", "eigen3"),
]
extra_link_args = []
libraries = []
library_dirs = []
...
# check if micromamba environment activated (CI)
    ...
    conda_prefix = os.path.join(os.environ["MAMBA_ROOT_PREFIX"], "envs", "snpkit")
else:
    conda_prefix = None
...
# Include snpkit and all submodules.
# This removes setuptools warning about missing namespace packages.
packages = ["snpkit"] + [
    f"snpkit.{submod}"
    for submod in find_namespace_packages("snpkit")
]
